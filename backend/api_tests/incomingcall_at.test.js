...
test("GET /api/incomingcall", async done => { 
    ...
    const getCall = await agent.get("/api/incomingcall").send().expect(200);
    done()
})
...
test("POST /api/incomingcall", async done => { 
    ...
    const getCall = await agent.post("/api/incomingcall").send(callDetails).expect(200);
    expect(getCall.text).toBe("Saved callername.");
    const getCallError = await agent.post("/api/incomingcall").send().expect(500);
    done()
})
...
test("DELETE /api/incomingcall", async done => { 
    ...
    const deleteCall = await agent.delete("/api/incomingcall").send(endCall).expect(200);
    expect(deleteCall.text).toBe("Cleared incoming call field.");
    const deleteCallError = await agent.delete("/api/incomingcall").send().expect(500);
    done()
})